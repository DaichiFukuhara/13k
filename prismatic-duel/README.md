...
| ローリング | L または C |
| ポーズ | Escape（試用中も可） |
| ポーズから再開 | Enter / Escape |
| 同じシードを1体目からやり直す | ポーズ中に R |
| 新しいシードへ | ポーズ中に N → タイトルで Enter |
| 撃破後の技選択 | A/D または左右キー |
| 選択した技を試す | T |
| 試用から技選択へ戻る | R |
| 技を確定して次へ | Enter |

撃破後のJ/Z連打では技選択画面に留まります。試用はTで開始し、試用中の攻撃にはJ/Zを使います。

パリィは横薙ぎ・突き・叩きつけ・突進に対応し、射撃は反射できます。落下攻撃（RAIN）は回避してください。
対応技は発生直前の11Fに予告の枠が白くなります。行動できる状態ならK/Xを押した瞬間から約0.2秒（0〜11F）パリィできます。
押しっぱなしでは再発動せず、失敗後には隙が残ります。攻撃中の硬直はキャンセルできません。
初期の角突きは7Fで発生。奪った技も各形状の出始めを2F早め、軽い技と重い技の差を保っています。
